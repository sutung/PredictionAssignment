...
to select the best model later. The purpose of this validation set is to
reduce the out of sample error (The error rate you get on a new data
set).
...
Remove the first five variables with identifications data from all
dataset include train set, validation set and test data.
...
using multiple cores. Cross validation can reduce the out of sample
error.
...
On top of the cross validation in Random Forest. We compare the accuracy
of the RF models and GBM models to reduce the out of sample error. The error
is expected to be less than 2%.

We compute the predictions of Random Forest(RF) model on validation set
and then compute the confusion matrix.The accuracy of the RF model is
99.85%.
